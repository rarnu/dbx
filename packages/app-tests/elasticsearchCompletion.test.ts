...
import {
  buildElasticsearchCompletionItems,
...
function applyCompletion(text: string, cursor: number, label: string): string {
  ...
  const item = buildElasticsearchCompletionItems(text, cursor, { indices }).find(
...
test("suggests Elasticsearch JSON DSL keys and snippets", () => {
  const keyItems = buildElasticsearchCompletionItems(
    'GET /orders/_search\n{\n  "qu',
    'GET /orders/_search\n{\n  "qu'.length,
  );
  assert.ok(keyItems.find((item) => item.label === '"query"'));

  const snippetItems = buildElasticsearchCompletionItems(
    'GET /orders/_search\n{\n  "match_',
    'GET /orders/_search\n{\n  "match_'.length,
  );
